...
module test_kiosk::test_kiosk {
  ...
  fun test_royalty() {
    ...
    next_tx(scenario_mut, ARTIST);
    {
      ...
      royalty_rule::add_rule(&mut policy, &policy_cap, 100);
    ...
    next_tx(scenario_mut, ALICE);
    {
      ...
      let royalty_amount = royalty_rule::royalty_amount(&policy, 100);
      let royalty = mint_for_testing(royalty_amount, ctx(scenario_mut));

      royalty_rule::pay_royalty(&mut policy, &mut request, royalty);
    ...
    next_tx(scenario_mut, ARTIST);
    {
      ...
      let policy = ts::take_shared<TransferPolicy<NFT>>(scenario_mut);
      let cap = ts::take_from_sender<TransferPolicyCap<NFT>>(scenario_mut);
      let royalty = transfer_policy::withdraw(&mut policy, &cap, option::none(), ctx(scenario_mut));
      assert_eq(royalty_rule::royalty_amount(&policy, 100), coin::value(&royalty));

      burn_for_testing(royalty);
