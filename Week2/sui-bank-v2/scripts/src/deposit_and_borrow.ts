...
(async () => {
  const getAccountForAddress = async (
    addr: string
  ...
  try {
    ...
    tx.moveCall({
      ...
      arguments: [tx.object(getId("bank::Bank")), account, coin],
    });
    ...
    const [price] = tx.moveCall({
      ...
      arguments: [
        ...
        tx.object(getId("oracle::OracleParams")),
        tx.object("0x6"), // clock
      ],
    });
    ...
    const [sui_dollar] = tx.moveCall({
      ...
      arguments: [
        account,
        tx.object(getId("lending::LendingParams")),
