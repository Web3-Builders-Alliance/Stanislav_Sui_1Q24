module sui_bank::bank {
    ...
    public fun swap_sui_dollar(cap: &mut CapWrapper, self: &mut Bank, coin_in: Coin<SUI_DOLLAR>, ctx: &mut TxContext): Coin<SUI> {
        let sui_dollar_amount = sui_dollar::burn(cap, coin_in);
        let sui_amount = (((sui_dollar_amount as u128) * 100 / EXCHANGE_RATE) as u64);
        // coin::from_balance(balance::split(&mut self.balance, sui_amount), ctx)
        coin::take(&mut self.balance, sui_amount, ctx)
    }
